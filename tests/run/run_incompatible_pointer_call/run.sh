...
set +e
OUTPUT="$("${BUILD_DIR}/SysyCC" "${INPUT_FILE}" --dump-ir 2>&1)"
RC=$?
set -e

if [[ ${RC} -ne 0 ]]; then
    echo "error: incompatible pointer call run case failed to compile" >&2
    echo "${OUTPUT}" >&2
    exit 1
fi

grep -Fq "semantic warning: function call argument uses incompatible pointer type" <<<"${OUTPUT}"
...
echo "verified: incompatible pointer call arguments warn and still lower through runtime execution"
