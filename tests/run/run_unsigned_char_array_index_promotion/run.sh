...
if ! grep -q 'zext i8 .* to i32' "${IR_FILE}"; then
    echo "error: expected unsigned char subscript to be zero-extended before GEP" >&2
    exit 1
fi
...
echo "verified: unsigned char array subscripts are integer-promoted before address calculation"
