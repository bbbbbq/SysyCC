...
if [[ ${RC} -ne 0 ]]; then
    echo "error: incompatible pointer return run case failed to compile" >&2
    echo "${OUTPUT}" >&2
    exit 1
fi
...
echo "verified: incompatible pointer returns warn and still lower through runtime execution"
