...
if [[ ${RC} -ne 0 ]]; then
    echo "error: incompatible pointer assignment run case failed to compile" >&2
    echo "${OUTPUT}" >&2
    exit 1
fi

grep -Fq "semantic warning: assignment between incompatible pointer types" <<<"${OUTPUT}"
...
echo "verified: incompatible pointer assignments warn and still lower through runtime execution"
