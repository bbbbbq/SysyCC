...
INPUT_FILE="${SCRIPT_DIR}/run_short_circuit_guard.sy"
...
PROGRAM_INPUT="${SCRIPT_DIR}/run_short_circuit_guard.in"
EXPECTED_OUTPUT="${SCRIPT_DIR}/run_short_circuit_guard.out"
...
grep -q 'logic\.rhs' "${IR_FILE}"
grep -q 'logic\.end' "${IR_FILE}"
grep -q 'sdiv i32' "${IR_FILE}"

echo "verified: short-circuit runtime test compiles, links, and avoids invalid evaluation"
