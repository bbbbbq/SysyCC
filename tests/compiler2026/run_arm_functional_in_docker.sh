...
usage() {
    cat <<'EOF'
Usage:
  ./tests/compiler2026/run_arm_functional_in_docker.sh [--image-tag tag] [--no-build] [run_arm_functional.sh args...]

Examples:
  ./tests/compiler2026/run_arm_functional_in_docker.sh 00_main
  ./tests/compiler2026/run_arm_functional_in_docker.sh --no-build --case-root /path/to/cases sample_case
EOF
}
...
compiler2025_docker_run_script \
    "${IMAGE_TAG}" "./tests/compiler2026/run_arm_functional.sh"
