...
source "${PROJECT_ROOT}/tests/compiler2025/compiler2025_docker_common.sh"

usage() {
    cat <<'EOF'
Usage:
  ./tests/compiler2026/run_functional_in_docker.sh [--image-tag tag] [--no-build] [run_functional.sh args...]

Examples:
  ./tests/compiler2026/run_functional_in_docker.sh --suite all
  ./tests/compiler2026/run_functional_in_docker.sh --no-build --suite functional 00_main
EOF
}
...
compiler2025_docker_run_script "${IMAGE_TAG}" "./tests/compiler2026/run_functional.sh"
