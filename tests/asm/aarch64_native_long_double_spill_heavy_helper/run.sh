...
test "$(grep -Ec '^  bl __addtf3$' "${ASM_FILE}")" -ge 3
grep -q '^  bl bump$' "${ASM_FILE}"
test "$(grep -Ec '^  ldr q[0-9]+, \[x29, #-[0-9]+\]$' "${ASM_FILE}")" -ge 2

object_status=0
build_aarch64_native_object "${ASM_FILE}" "${OBJ_FILE}" || object_status=$?
if [[ "${object_status}" -eq 0 ]]; then
    assert_file_nonempty "${OBJ_FILE}"
    assert_aarch64_relocations "${OBJ_FILE}" '__addtf3|bump'
else
    if [[ "${object_status}" -ne 125 ]]; then
        exit "${object_status}"
    fi
    echo "skipped: no AArch64 cross compiler for spill-heavy long double object smoke"
fi
...
if [[ "${link_status}" -eq 0 ]]; then
    ...
    run_status=0
    run_aarch64_native_smoke_if_available "${BIN_FILE}" "42" $'32\n' || run_status=$?
    if [[ "${run_status}" -eq 125 ]]; then
        echo "skipped: no qemu/sysroot for spill-heavy long double execute smoke"
    ...
    echo "skipped: no AArch64 sysroot/linker for spill-heavy long double link smoke"
fi

echo "verified: spill-heavy long double helper path still lowers, assembles, and optionally runs"
