...
grep -Eq '^  add x[0-9]+, x[0-9]+, x[0-9]+, lsl #3$' "${ASM_FILE}"
grep -Eq '^  add x[0-9]+, x[0-9]+, x[0-9]+, lsl #2$' "${ASM_FILE}"

echo "verified: native AArch64 asm lowers mixed pointer/array/struct GEP chains without illegal width combinations"
