...
if [[ "${object_status}" -eq 0 ]]; then
    ...
    echo "skipped: no AArch64 cross compiler for indirect-call smoke"
    exit 0
fi
...
if [[ "${link_status}" -eq 0 ]]; then
    ...
    run_status=0
    run_aarch64_native_smoke_if_available "${BIN_FILE}" "22" || run_status=$?
    if [[ "${run_status}" -eq 125 ]]; then
        echo "skipped: no qemu/sysroot for indirect-call execute smoke"
    ...
    echo "skipped: no AArch64 sysroot/linker for indirect-call link smoke"
fi

echo "verified: indirect-call spill lowering preserves return-value direction through asm and smoke execution"
