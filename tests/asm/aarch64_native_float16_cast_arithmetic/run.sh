...
grep -Eq '^  scvtf s[0-9]+, w[0-9]+$' "${ASM_FILE}"
grep -Eq '^  fcvt h[0-9]+, s[0-9]+$' "${ASM_FILE}"
grep -Eq '^  fadd s[0-9]+, s[0-9]+, s[0-9]+$' "${ASM_FILE}"
grep -Eq '^  fcvtzs w[0-9]+, s[0-9]+$' "${ASM_FILE}"
...
if [[ "${object_status}" -eq 0 ]]; then
    ...
    echo "skipped: no AArch64 cross compiler + sysroot for _Float16 object smoke"
fi
...
if [[ "${link_status}" -eq 0 ]]; then
    ...
    run_status=0
    run_aarch64_native_smoke_if_available "${BIN_FILE}" "7" $'5\n' || run_status=$?
    if [[ "${run_status}" -eq 125 ]]; then
        echo "skipped: no qemu/sysroot for _Float16 execute smoke"
    ...
    echo "skipped: no AArch64 sysroot/linker for _Float16 link smoke"
fi

echo "verified: native AArch64 asm legalizes _Float16 arithmetic through float32 temps"
