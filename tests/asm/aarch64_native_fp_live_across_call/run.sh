...
grep -Eq '^  str d8, \[x29, #-[0-9]+\]$' "${ASM_FILE}"
grep -q '^  bl inc$' "${ASM_FILE}"
grep -Eq '^[[:space:]]*fadd d[0-9]+, d[0-9]+, d[0-9]+$' "${ASM_FILE}"
...
if [[ "${object_status}" -eq 0 ]]; then
    ...
    echo "skipped: no AArch64 cross compiler for FP live-across-call object smoke"
fi
...
if [[ "${link_status}" -eq 0 ]]; then
    ...
    run_status=0
    run_aarch64_native_smoke_if_available "${BIN_FILE}" "62" || run_status=$?
    if [[ "${run_status}" -eq 125 ]]; then
        echo "skipped: no qemu/sysroot for FP live-across-call execute smoke"
    ...
    echo "skipped: no AArch64 sysroot/linker for FP live-across-call smoke"
fi

echo "verified: live-across-call floating values use callee-saved float registers"
