...
test "$(grep -Ec '^[[:space:]]*sxtw x[0-9]+, w[0-9]+$' "${ASM_FILE}")" -ge 5
test "$(grep -Ec '^[[:space:]]*add x[0-9]+, x[0-9]+, x[0-9]+, lsl #2$' "${ASM_FILE}")" -ge 5
test "$(grep -Ec '^[[:space:]]*ldr w[0-9]+, \[x[0-9]+(, #0)?\]$' "${ASM_FILE}")" -ge 5
