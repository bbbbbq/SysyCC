...
grep -q '^  bl __addtf3$' "${ASM_FILE}"
grep -Eq '^  ldr q[0-9]+, \[x29, #-[0-9]+\]$' "${ASM_FILE}"

object_status=0
build_aarch64_native_object "${ASM_FILE}" "${OBJ_FILE}" || object_status=$?
if [[ "${object_status}" -eq 0 ]]; then
    assert_file_nonempty "${OBJ_FILE}"
    assert_aarch64_relocations "${OBJ_FILE}" '__floatsitf|__addtf3'
else
    if [[ "${object_status}" -ne 125 ]]; then
        exit "${object_status}"
    fi
    echo "skipped: no AArch64 cross compiler for long double add object smoke"
fi
...
if [[ "${link_status}" -eq 0 ]]; then
    ...
    run_status=0
    run_aarch64_native_smoke_if_available "${BIN_FILE}" "42" $'41\n' || run_status=$?
    if [[ "${run_status}" -eq 125 ]]; then
        echo "skipped: no qemu/sysroot for long double add execute smoke"
    ...
    echo "skipped: no AArch64 sysroot/linker for long double add smoke"
fi

echo "verified: native AArch64 asm lowers long double arithmetic through libgcc-style helpers"
