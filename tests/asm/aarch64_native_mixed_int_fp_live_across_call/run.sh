...
grep -Eq '^[[:space:]]*str x(19|20|21|22|23), \[x29, #-[0-9]+\]$' "${ASM_FILE}"
grep -Eq '^[[:space:]]*str d(8|9|10|11|12|13|14|15), \[x29, #-[0-9]+\]$' "${ASM_FILE}"
...
if [[ "${object_status}" -eq 0 ]]; then
    ...
    echo "skipped: no AArch64 cross compiler + sysroot for mixed int/fp object smoke"
fi
...
if [[ "${link_status}" -eq 0 ]]; then
    ...
    run_status=0
    run_aarch64_native_smoke_if_available "${BIN_FILE}" "14" $'4\n5\n1\n2\n' || run_status=$?
    if [[ "${run_status}" -eq 125 ]]; then
        echo "skipped: no qemu/sysroot for mixed int/fp execute smoke"
    ...
    echo "skipped: no AArch64 sysroot/linker for mixed int/fp smoke"
fi

echo "verified: mixed integer and floating live-across-call values survive through callee-saved registers"
