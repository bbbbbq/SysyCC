...
if [[ "${object_status}" -eq 0 ]]; then
    ...
    echo "skipped: no AArch64 cross compiler for direct-call smoke"
    exit 0
fi
...
if [[ "${link_status}" -eq 0 ]]; then
    ...
    if [[ "${run_status}" -eq 125 ]]; then
        echo "skipped: no qemu/sysroot for direct-call execute smoke"
    ...
    echo "skipped: no AArch64 sysroot/linker for direct-call link smoke"
fi

echo "verified: direct-call spill lowering preserves return-value direction through asm and smoke execution"
