...
INPUT_FILE="${SCRIPT_DIR}/nullable_function_pointer_parameter_prototype.sy"
...
grep -q 'ANNOTATION_IDENT _LIBC_COUNT' "${BUILD_DIR}/intermediate_results/${TEST_NAME}.parse.txt"
grep -q 'NULLABILITY _Nullable' "${BUILD_DIR}/intermediate_results/${TEST_NAME}.parse.txt"

echo "verified: parser accepts nullability-marked function pointer parameters"
