...
INPUT_FILE="${SCRIPT_DIR}/sizeof_type_name.sy"
...
grep -q "^KwSizeof sizeof " \
    "${BUILD_DIR}/intermediate_results/${TEST_NAME}.tokens.txt"
grep -q "sizeof_type_expr" \
    "${BUILD_DIR}/intermediate_results/${TEST_NAME}.parse.txt"
grep -q "abstract_array_suffix_list" \
    "${BUILD_DIR}/intermediate_results/${TEST_NAME}.parse.txt"

echo "verified: parser accepts sizeof(type-name)"
