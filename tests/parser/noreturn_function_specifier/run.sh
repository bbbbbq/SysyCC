...
INPUT_FILE="${SCRIPT_DIR}/noreturn_function_specifier.sy"
...
grep -q '^KwNoreturn _Noreturn ' \
    "${BUILD_DIR}/intermediate_results/${TEST_NAME}.tokens.txt"
grep -q 'NORETURN _Noreturn' \
    "${BUILD_DIR}/intermediate_results/${TEST_NAME}.parse.txt"

echo "verified: parser accepts C11 _Noreturn function specifiers"
