...
int main(int argc, char **argv) {
    if (argc != 3) {
        std::cerr
            << "usage: aarch64_codegen_api_global_pointer_constant <input.ll> <output.o>\n";
        return 1;
    }
    ...
    if (asm_result.asm_text.find("seed:") == std::string::npos ||
        asm_result.asm_text.find("ptr_global:") == std::string::npos ||
        asm_result.asm_text.find("ptr_array:") == std::string::npos ||
