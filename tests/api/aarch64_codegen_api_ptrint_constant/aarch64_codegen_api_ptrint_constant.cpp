...
int main(int argc, char **argv) {
    if (argc != 3) {
        std::cerr
            << "usage: aarch64_codegen_api_ptrint_constant <input.ll> <output.o>\n";
        return 1;
    }
    ...
    if (asm_result.asm_text.find("seed:") == std::string::npos ||
        asm_result.asm_text.find("seed_addr:") == std::string::npos ||
        asm_result.asm_text.find("null_ptr:") == std::string::npos ||
        asm_result.asm_text.find("pair:") == std::string::npos ||
        asm_result.asm_text.find("field_addr:") == std::string::npos ||
        asm_result.asm_text.find(".globl get_seed_addr") == std::string::npos ||
        asm_result.asm_text.find(".globl get_null_ptr") == std::string::npos) {
