...
int main(int argc, char **argv) {
    if (argc != 3) {
        std::cerr << "usage: aarch64_codegen_api_undef_poison_constant <input.ll> <output.o>\n";
        return 1;
    }
    ...
    if (asm_result.asm_text.find("u_i32:") == std::string::npos ||
        asm_result.asm_text.find("p_i64:") == std::string::npos ||
        asm_result.asm_text.find("u_ptr:") == std::string::npos ||
        asm_result.asm_text.find("p_vec:") == std::string::npos ||
        asm_result.asm_text.find("picked:") == std::string::npos ||
        asm_result.asm_text.find("cmp_from_poison:") == std::string::npos ||
        asm_result.asm_text.find("lane_psel:") == std::string::npos ||
        asm_result.asm_text.find(".globl main") == std::string::npos) {
