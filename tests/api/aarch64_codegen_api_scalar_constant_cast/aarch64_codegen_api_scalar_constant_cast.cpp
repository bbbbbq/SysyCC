...
int main(int argc, char **argv) {
    if (argc != 3) {
        std::cerr << "usage: aarch64_codegen_api_scalar_constant_cast <input.ll> <output.o>\n";
        return 1;
    }
    ...
    if (asm_result.asm_text.find("trunc_i32:") == std::string::npos ||
        asm_result.asm_text.find("zext_i64:") == std::string::npos ||
        asm_result.asm_text.find("sext_i64:") == std::string::npos ||
        asm_result.asm_text.find("flt32:") == std::string::npos ||
        asm_result.asm_text.find("dbl_to_i32:") == std::string::npos ||
        asm_result.asm_text.find("bit_bits:") == std::string::npos ||
