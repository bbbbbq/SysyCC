...
int main(int argc, char **argv) {
    if (argc != 3) {
        std::cerr << "usage: aarch64_codegen_api_rich_syntax <input.ll> <output.o>\n";
        return 1;
    }
    ...
    if (asm_result.asm_text.find(".globl sum_pair") == std::string::npos ||
        asm_result.asm_text.find("pair_global:") == std::string::npos ||
        asm_result.asm_text.find("bl sum_pair") == std::string::npos ||
        asm_result.asm_text.find(".p2align 3") == std::string::npos ||
        asm_result.asm_text.find("sum_pair_alias") != std::string::npos ||
        asm_result.asm_text.find("pair_alias") != std::string::npos) {
