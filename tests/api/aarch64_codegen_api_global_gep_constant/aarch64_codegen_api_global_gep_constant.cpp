...
int main(int argc, char **argv) {
    if (argc != 3) {
        std::cerr
            << "usage: aarch64_codegen_api_global_gep_constant <input.ll> <output.o>\n";
        return 1;
    }
    ...
    if (asm_result.asm_text.find("items:") == std::string::npos ||
        asm_result.asm_text.find("plus_one:") == std::string::npos ||
        asm_result.asm_text.find("pair:") == std::string::npos ||
        asm_result.asm_text.find("payload:") == std::string::npos ||
        asm_result.asm_text.find("q:") == std::string::npos ||
        asm_result.asm_text.find("g_ptr:") == std::string::npos ||
        asm_result.asm_text.find("field_ptrs:") == std::string::npos ||
        asm_result.asm_text.find("items") == std::string::npos ||
        asm_result.asm_text.find("pair") == std::string::npos ||
        asm_result.asm_text.find("payload") == std::string::npos) {
