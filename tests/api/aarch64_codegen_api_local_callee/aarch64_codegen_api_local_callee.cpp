...
int main(int argc, char **argv) {
    if (argc != 3) {
        std::cerr
            << "usage: aarch64_codegen_api_local_callee <input.ll> <output.o>\n";
        return 1;
    }
    ...
    if (asm_result.asm_text.find(".globl add") == std::string::npos ||
        asm_result.asm_text.find("blr ") == std::string::npos ||
        asm_result.asm_text.find("%callee") != std::string::npos) {
