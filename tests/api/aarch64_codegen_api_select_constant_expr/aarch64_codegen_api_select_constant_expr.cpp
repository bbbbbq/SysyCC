...
int main(int argc, char **argv) {
    if (argc != 3) {
        std::cerr << "usage: aarch64_codegen_api_select_constant_expr <input.ll> <output.o>\n";
        return 1;
    }
    ...
    if (asm_result.asm_text.find("picked:") == std::string::npos ||
        asm_result.asm_text.find("seed:") == std::string::npos ||
        asm_result.asm_text.find("picked_ptr:") == std::string::npos ||
        asm_result.asm_text.find("vec_sel:") == std::string::npos ||
        asm_result.asm_text.find("lane_from_select:") == std::string::npos ||
