...
int main(int argc, char **argv) {
    if (argc != 3) {
        std::cerr << "usage: aarch64_codegen_api_addrspace_cast <input.ll> <output.o>\n";
        return 1;
    }
    ...
    if (asm_result.asm_text.find("seed:") == std::string::npos ||
        asm_result.asm_text.find("seed_as1:") == std::string::npos ||
        asm_result.asm_text.find(".globl roundtrip") == std::string::npos ||
