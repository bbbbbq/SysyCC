...
SOURCE_FILE="${SCRIPT_DIR}/aarch64_codegen_api_addrspace_cast.cpp"
LL_FILE="${SCRIPT_DIR}/aarch64_codegen_api_addrspace_cast.ll"
TEST_BIN="${CASE_BUILD_DIR}/aarch64_codegen_api_addrspace_cast"
OBJECT_FILE="${CASE_BUILD_DIR}/aarch64_codegen_api_addrspace_cast.o"
EXECUTABLE_FILE="${CASE_BUILD_DIR}/aarch64_codegen_api_addrspace_cast.bin"
...
"${READELF_TOOL}" -s "${OBJECT_FILE}" | grep -q ' seed_as1$'
"${READELF_TOOL}" -s "${OBJECT_FILE}" | grep -q ' roundtrip$'
...
echo "verified: importer accepts ptr addrspace syntax and lowers addrspacecast through the AArch64 asm/object bridge"
