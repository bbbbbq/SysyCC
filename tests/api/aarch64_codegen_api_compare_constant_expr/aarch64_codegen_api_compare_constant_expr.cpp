...
int main(int argc, char **argv) {
    if (argc != 3) {
        std::cerr << "usage: aarch64_codegen_api_compare_constant_expr <input.ll> <output.o>\n";
        return 1;
    }
    ...
    if (asm_result.asm_text.find("icmp_gt:") == std::string::npos ||
        asm_result.asm_text.find("fcmp_eq:") == std::string::npos ||
        asm_result.asm_text.find("ptr_eq:") == std::string::npos ||
        asm_result.asm_text.find("ptr_ne:") == std::string::npos ||
        asm_result.asm_text.find("vec_cmp:") == std::string::npos ||
        asm_result.asm_text.find("lane_from_cmp_select:") == std::string::npos ||
