...
int main(int argc, char **argv) {
    if (argc != 3) {
        std::cerr << "usage: aarch64_codegen_api_indirect_branch_phi <input.ll> <output.o>\n";
        return 1;
    }
    ...
    if (asm_result.asm_text.find(".globl dispatch") == std::string::npos ||
        asm_result.asm_text.find(".Ldispatch_entry_to_target_phi") ==
            std::string::npos ||
        asm_result.asm_text.find("b.eq .Ldispatch_entry_to_target_phi") ==
            std::string::npos) {
