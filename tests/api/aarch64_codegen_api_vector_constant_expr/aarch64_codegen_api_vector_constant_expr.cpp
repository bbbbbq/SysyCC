...
int main(int argc, char **argv) {
    if (argc != 3) {
        std::cerr << "usage: aarch64_codegen_api_vector_constant_expr <input.ll> <output.o>\n";
        return 1;
    }
    ...
    if (asm_result.asm_text.find("lane:") == std::string::npos ||
        asm_result.asm_text.find("vec_insert:") == std::string::npos ||
        asm_result.asm_text.find("vec_shuffle:") == std::string::npos ||
        asm_result.asm_text.find("lane_nested:") == std::string::npos ||
