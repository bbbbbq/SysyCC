...
void test_unswitches_loop_body_invariant_condition() {
    ...
    auto *counter_cmp = header->create_instruction<CoreIrCompareInst>(
        CoreIrComparePredicate::SignedLess, i1_type, "counter.cmp",
        counter_load, three);
    header->create_instruction<CoreIrCondJumpInst>(void_type, counter_cmp, body,
                                                   exit);
    ...
    body->create_instruction<CoreIrCondJumpInst>(void_type, flag_cmp,
                                                 then_block, latch);
    ...
    latch->create_instruction<CoreIrStoreInst>(void_type, counter_next,
                                               counter_slot);
...
void test_unswitches_loop_body_compare_chain_from_invariant_access_path() {
    ...
    auto *module = context->create_module<CoreIrModule>(
        "simple_unswitch_body_access_path");
    auto *function =
        module->create_function<CoreIrFunction>("main", function_type, false);
    auto *counter_slot =
        function->create_stack_slot<CoreIrStackSlot>("counter", i32_type, 4);
    ...
    header->create_instruction<CoreIrCondJumpInst>(void_type, counter_cmp, body,
                                                   exit);
    auto *loop_state_addr =
        body->create_instruction<CoreIrAddressOfStackSlotInst>(
    ...
    auto *flag_active = body->create_instruction<CoreIrCompareInst>(
        CoreIrComparePredicate::NotEqual, i1_type, "flag.active", flag_load,
        zero);
    auto *flag_large = body->create_instruction<CoreIrCompareInst>(
        CoreIrComparePredicate::SignedGreater, i1_type, "flag.large", flag_sum,
        one);
    auto *combined = body->create_instruction<CoreIrBinaryInst>(
        CoreIrBinaryOpcode::And, i1_type, "flag.combined", flag_active,
        flag_large);
    body->create_instruction<CoreIrCondJumpInst>(void_type, combined,
                                                 then_block, latch);
    then_block->create_instruction<CoreIrJumpInst>(void_type, latch);
    auto *counter_reload = latch->create_instruction<CoreIrLoadInst>(
        i32_type, "counter.reload", counter_slot);
...
void test_unswitch_preserves_gep_source_type_when_cloning_void_base() {
    ...
    header->create_instruction<CoreIrCondJumpInst>(void_type, iv_cmp, body,
                                                   exit);
...
void test_unswitches_outer_loop_even_with_nested_inner_loop_phi() {
    ...
    auto *outer_body =
        function->create_basic_block<CoreIrBasicBlock>("outer.body");
    auto *then_block = function->create_basic_block<CoreIrBasicBlock>("then");
    auto *inner_header =
    ...
    auto *outer_exit =
        function->create_basic_block<CoreIrBasicBlock>("outer.exit");
    auto *zero = context->create_constant<CoreIrConstantInt>(i32_type, 0);
    auto *one = context->create_constant<CoreIrConstantInt>(i32_type, 1);
    auto *two = context->create_constant<CoreIrConstantInt>(i32_type, 2);
    auto *three = context->create_constant<CoreIrConstantInt>(i32_type, 3);
    ...
    auto *counter_cmp = outer_header->create_instruction<CoreIrCompareInst>(
        CoreIrComparePredicate::SignedLess, i1_type, "counter.cmp",
        counter_load, three);
    outer_header->create_instruction<CoreIrCondJumpInst>(
        void_type, counter_cmp, outer_body, outer_exit);
    auto *flag_load = outer_body->create_instruction<CoreIrLoadInst>(
        i32_type, "flag.load", flag_slot);
    auto *flag_cmp = outer_body->create_instruction<CoreIrCompareInst>(
        CoreIrComparePredicate::NotEqual, i1_type, "flag.cmp", flag_load, zero);
    outer_body->create_instruction<CoreIrCondJumpInst>(void_type, flag_cmp,
                                                       then_block, inner_exit);
    ...
    inner_body->create_instruction<CoreIrStoreInst>(void_type, sum_next,
                                                    sum_slot);
    ...
    auto *counter_next = inner_exit->create_instruction<CoreIrBinaryInst>(
        CoreIrBinaryOpcode::Add, i32_type, "counter.next", counter_reload, one);
    inner_exit->create_instruction<CoreIrStoreInst>(void_type, counter_next,
                                                    counter_slot);
...
void test_stops_unswitch_after_depth_budget() {
    ...
    auto *entry = function->create_basic_block<CoreIrBasicBlock>("entry");
    auto *header =
        function->create_basic_block<CoreIrBasicBlock>("header.unsw.true");
    auto *body = function->create_basic_block<CoreIrBasicBlock>("body");
    auto *then_block = function->create_basic_block<CoreIrBasicBlock>("then");
    auto *latch = function->create_basic_block<CoreIrBasicBlock>("latch");
    auto *exit = function->create_basic_block<CoreIrBasicBlock>("exit");
...
void test_skips_loop_body_unswitch_when_successor_has_phi() {
    ...
    auto *join_phi =
        join->create_instruction<CoreIrPhiInst>(i32_type, "join.value");
    auto *counter_reload = join->create_instruction<CoreIrLoadInst>(
        i32_type, "counter.reload", counter_slot);
    auto *counter_next = join->create_instruction<CoreIrBinaryInst>(
        CoreIrBinaryOpcode::Add, i32_type, "counter.next", counter_reload, one);
    join->create_instruction<CoreIrStoreInst>(void_type, counter_next,
                                              counter_slot);
...
void test_skips_loop_body_unswitch_when_condition_has_multiple_predecessors() {
    auto context = std::make_unique<CoreIrContext>();
    auto *void_type = context->create_type<CoreIrVoidType>();
    auto *i1_type = context->create_type<CoreIrIntegerType>(1);
    auto *i32_type = context->create_type<CoreIrIntegerType>(32);
    auto *function_type = context->create_type<CoreIrFunctionType>(
        i32_type, std::vector<const CoreIrType *>{}, false);
    auto *module =
        context->create_module<CoreIrModule>("simple_unswitch_multi_pred");
    auto *function =
        module->create_function<CoreIrFunction>("main", function_type, false);
    auto *counter_slot =
        function->create_stack_slot<CoreIrStackSlot>("counter", i32_type, 4);
    ...
    auto *body = function->create_basic_block<CoreIrBasicBlock>("body");
    auto *condition =
        function->create_basic_block<CoreIrBasicBlock>("condition");
    auto *then_block = function->create_basic_block<CoreIrBasicBlock>("then");
    auto *latch = function->create_basic_block<CoreIrBasicBlock>("latch");
    auto *exit = function->create_basic_block<CoreIrBasicBlock>("exit");
    auto *zero = context->create_constant<CoreIrConstantInt>(i32_type, 0);
    ...
    header->create_instruction<CoreIrCondJumpInst>(void_type, counter_cmp, body,
                                                   exit);
    body->create_instruction<CoreIrJumpInst>(void_type, condition);
    auto *flag_load = condition->create_instruction<CoreIrLoadInst>(
        i32_type, "flag.load", flag_slot);
    auto *flag_cmp = condition->create_instruction<CoreIrCompareInst>(
        CoreIrComparePredicate::NotEqual, i1_type, "flag.cmp", flag_load, zero);
    condition->create_instruction<CoreIrCondJumpInst>(void_type, flag_cmp,
                                                      then_block, latch);
    then_block->create_instruction<CoreIrJumpInst>(void_type, condition);
    auto *counter_reload = latch->create_instruction<CoreIrLoadInst>(
        i32_type, "counter.reload", counter_slot);
    auto *counter_next = latch->create_instruction<CoreIrBinaryInst>(
        CoreIrBinaryOpcode::Add, i32_type, "counter.next", counter_reload, one);
...
int main() {
    ...
    test_skips_loop_body_unswitch_when_condition_has_multiple_predecessors();
    return 0;
}
