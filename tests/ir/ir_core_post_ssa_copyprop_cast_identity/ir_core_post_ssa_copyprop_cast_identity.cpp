...
int main() {
    ...
    auto *module = context->create_module<CoreIrModule>(
        "ir_core_post_ssa_copyprop_cast_identity");
    auto *function =
        module->create_function<CoreIrFunction>("main", function_type, false);
    auto *entry = function->create_basic_block<CoreIrBasicBlock>("entry");
    auto *seven = context->create_constant<CoreIrConstantInt>(i32_type, 7);

    auto *identity_cast = entry->create_instruction<CoreIrCastInst>(
        CoreIrCastKind::ZeroExtend, i32_type, "identity", seven);
    entry->create_instruction<CoreIrReturnInst>(void_type, identity_cast);
