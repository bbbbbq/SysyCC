...
int main() {
    ...
    auto *module = context->create_module<CoreIrModule>("ir_core_const_fold_pass");
    auto *function =
        module->create_function<CoreIrFunction>("main", function_type, false);
    auto *entry = function->create_basic_block<CoreIrBasicBlock>("entry");
    auto *true_block = function->create_basic_block<CoreIrBasicBlock>("true_block");
    auto *false_block = function->create_basic_block<CoreIrBasicBlock>("false_block");
    ...
    assert(text.find("jmp %true_block") != std::string::npos);
    return 0;
}
