...
int main() {
    ...
    auto *module = context->create_module<CoreIrModule>("ir_core_phi_lowering");
    auto *function =
    ...
    assert(compiler_context.get_ir_result() != nullptr);
    const std::string &text = compiler_context.get_ir_result()->get_text();
    assert(text.find("%t0 = phi i32 [ 1, %left ], [ 2, %right ]") !=
           std::string::npos);
    return 0;
}
