...
int main(int argc, char **argv) {
    ...
    assert(contains("module ir_core_builder_pointer_deref_assign\n"));
    assert(contains("stackslot %value.addr : i32, align 4\n"));
    assert(contains("stackslot %ptr.addr : i32*, align 8\n"));
    assert(contains("addr_of_stackslot i32* %value.addr"));
    assert(contains("addr_of_stackslot i32** %ptr.addr"));
    assert(contains("load i32*"));
    assert(contains("store i32 7"));
