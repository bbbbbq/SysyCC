...
int main() {
    ...
    auto *module = context->create_module<CoreIrModule>("ir_core_alias_analysis");
    auto *global =
        module->create_global<CoreIrGlobal>("g", i32_type, nullptr, false, false);
    auto *function =
        module->create_function<CoreIrFunction>("main", function_type, false);
    auto *parameter = function->create_parameter<CoreIrParameter>(ptr_i32_type, "arg");
    ...
    auto *arg_gep = entry->create_instruction<CoreIrGetElementPtrInst>(
        ptr_i32_type, "arg_gep", parameter,
        std::vector<CoreIrValue *>{one});
    auto *dyn_index = entry->create_instruction<CoreIrLoadInst>(
        i32_type, "dyn", parameter);
    auto *unknown_gep = entry->create_instruction<CoreIrGetElementPtrInst>(
        ptr_i32_type, "unknown_gep", parameter,
    ...
    assert(result.alias_values(arg_gep, unknown_gep) == CoreIrAliasKind::MayAlias);
    return 0;
}
