...
int main(int argc, char **argv) {
    ...
    assert(contains("module ir_core_builder_do_while\n"));
    assert(contains("func @main() -> i32 {\n"));
    assert(contains("stackslot %value.addr : i32, align 4\n"));
    assert(contains("dowhile.body0:\n"));
    assert(contains("dowhile.cond0:\n"));
    assert(contains("dowhile.end0:\n"));
    ...
    assert(contains("jmp %dowhile.cond0"));
    assert(contains("br i32 %"));
