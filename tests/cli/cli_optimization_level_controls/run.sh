...
grep -Fq "add i32 1, 2" "${IR_O0}"
if grep -Fq "add i32 1, 2" "${IR_O1}"; then
    echo "error: -O1 still emitted the un-folded integer add" >&2
    exit 1
fi
grep -Eq 'ret i32 3$|ret i32 3[^0-9]' "${IR_O1}"
