...
VERSION_OUTPUT="$("${BUILD_DIR}/compiler" --version 2>&1)"
grep -Fq "compiler version 0.1.0" <<<"${VERSION_OUTPUT}"

set +e
VERBOSE_OUTPUT="$("${BUILD_DIR}/compiler" -v -fsyntax-only "${INPUT_FILE}" 2>&1)"
VERBOSE_RC=$?
set -e
...
grep -Fq "compiler version 0.1.0" <<<"${VERBOSE_OUTPUT}"
