...
| compiler builtins | `tests/run/run_builtin_alloca_stack_smoke`, `tests/run/run_builtin_clz_smoke`, `tests/run/run_builtin_ctz_smoke`, `tests/run/run_builtin_prefetch_smoke`, `tests/run/run_atomic_builtin_smoke` | Covers `__builtin_alloca` lowering to LLVM stack allocation plus GCC bit-scan, prefetch, and atomic builtins used by real-project bitmap/hash-table/thread code. |
