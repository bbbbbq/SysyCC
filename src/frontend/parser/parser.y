...
%token <node> INVALID
%token <node> CONST VOLATILE EXTERN STATIC REGISTER ATTRIBUTE ASM EXTENSION INLINE NORETURN RESTRICT NULLABILITY LONG SIGNED SHORT UNSIGNED INT CHAR VOID FLOAT DOUBLE FLOAT16 IF ELSE WHILE FOR DO SWITCH CASE DEFAULT BREAK CONTINUE GOTO RETURN STRUCT UNION ENUM TYPEDEF SIZEOF TYPEOF ALIGNAS TYPE_NAME
