...
bool expr_is_obviously_nonzero_constant(const Expr *expr) {
    ...
    case AstKind::IntegerLiteralExpr:
        return static_cast<const IntegerLiteralExpr *>(expr)
                   ->get_value_text() != "0";
    case AstKind::CharLiteralExpr:
        return static_cast<const CharLiteralExpr *>(expr)->get_value_text() !=
               "'\\0'";
    default:
        return false;
    }
...
void emit_unused_symbol_warnings(SemanticContext &semantic_context) {
    SemanticModel &semantic_model = semantic_context.get_semantic_model();
    for (const SemanticSymbol *symbol :
         semantic_context.get_function_local_symbols()) {
        if (symbol == nullptr || symbol->get_decl_node() == nullptr ||
            symbol->get_name().empty()) {
            continue;
        }
        const std::size_t use_count =
            semantic_model.get_symbol_use_count(symbol);
    ...
    for (const auto &decl : translation_unit->get_top_level_decls()) {
        if (decl->get_kind() == AstKind::FunctionDecl) {
            analyze_function_decl(
                decl.get(), semantic_context, scope_stack, type_resolver,
                conversion_checker, decl_analyzer, stmt_analyzer,
                attribute_analyzer, static_function_candidates);
            continue;
        }
...
const SemanticType *SemanticAnalyzer::build_function_type(
    ...
    const SemanticType *return_type = type_resolver.resolve_type(
        function_decl->get_return_type(), semantic_context, &scope_stack);
    std::vector<const SemanticType *> parameter_types;
    for (const auto &parameter : function_decl->get_parameters()) {
        const auto *param_decl =
            static_cast<const ParamDecl *>(parameter.get());
    ...
    return semantic_context.get_semantic_model().own_type(
        std::make_unique<FunctionSemanticType>(
            return_type, parameter_types, function_decl->get_is_variadic()));
}

void SemanticAnalyzer::analyze_function_decl(
    const Decl *decl, SemanticContext &semantic_context,
    ScopeStack &scope_stack, const TypeResolver &type_resolver,
    const ConversionChecker &conversion_checker,
    const DeclAnalyzer &decl_analyzer, const StmtAnalyzer &stmt_analyzer,
    ...
    SemanticModel &semantic_model = semantic_context.get_semantic_model();
    decl_analyzer.analyze_inline_type_declarations(
        function_decl->get_return_type(), semantic_context, scope_stack);
    for (const auto &parameter : function_decl->get_parameters()) {
        const auto *param_decl =
            static_cast<const ParamDecl *>(parameter.get());
        if (param_decl != nullptr) {
            decl_analyzer.analyze_inline_type_declarations(
                param_decl->get_declared_type(), semantic_context, scope_stack);
        }
    }
    const SemanticType *function_type = build_function_type(
        function_decl, semantic_context, type_resolver, scope_stack);
    ...
    if (const SemanticSymbol *existing_symbol =
        ...
        function_symbol =
            semantic_model.own_symbol(std::make_unique<SemanticSymbol>(
                SymbolKind::Function, function_decl->get_name(), function_type,
                function_decl));
        function_defined =
    ...
    semantic_context.set_current_return_type(
        resolved_function_type != nullptr
            ? resolved_function_type->get_return_type()
            : nullptr);
    semantic_context.begin_function_labels();
    for (const auto &parameter : function_decl->get_parameters()) {
        decl_analyzer.analyze_decl(parameter.get(), semantic_context,
                                   scope_stack);
    }
