...
const SemanticType *
get_float_literal_semantic_type(const FloatLiteralExpr *expr,
...
const SemanticType *get_builtin_semantic_type(SemanticModel &semantic_model,
    ...
    return semantic_model.own_type(std::make_unique<BuiltinSemanticType>(name));
}
...
const SemanticType *
get_unsigned_int_semantic_type(SemanticModel &semantic_model) {
    return get_builtin_semantic_type(semantic_model, "unsigned int");
}

const SemanticType *
get_unsigned_short_semantic_type(SemanticModel &semantic_model) {
    return get_builtin_semantic_type(semantic_model, "unsigned short");
}

const SemanticType *
get_unsigned_char_semantic_type(SemanticModel &semantic_model) {
    return get_builtin_semantic_type(semantic_model, "unsigned char");
}

const SemanticType *
get_unsigned_long_long_semantic_type(SemanticModel &semantic_model) {
...
const SemanticType *
get_unsigned_long_semantic_type(SemanticModel &semantic_model) {
...
bool fits_unsigned_32(unsigned long long magnitude) {
    return magnitude <= 0xFFFFFFFFULL;
}
...
const SemanticType *
get_integer_literal_semantic_type(const IntegerLiteralExpr *expr,
                                  SemanticModel &semantic_model) {
    if (expr == nullptr) {
        return nullptr;
    }
...
std::string
get_compound_assignment_binary_operator(const std::string &operator_text) {
    if (operator_text == "+=") {
        return "+";
    }
    if (operator_text == "-=") {
...
const SemanticType *
get_unsigned_equivalent_integer_type(const SemanticType *type,
                                     SemanticModel &semantic_model) {
    const SemanticType *unqualified_type = strip_qualifiers(type);
    if (unqualified_type == nullptr ||
...
const SemanticType *
get_common_integer_type(const SemanticType *lhs, const SemanticType *rhs,
                        SemanticModel &semantic_model,
    ...
    const SemanticType *signed_type =
        lhs_is_signed ? lhs_promoted : rhs_promoted;
...
const SemanticType *
get_common_arithmetic_type(const SemanticType *lhs, const SemanticType *rhs,
                           SemanticModel &semantic_model,
                           const ConversionChecker &conversion_checker) {
...
get_callable_function_type(const SemanticType *type) {
    type = strip_qualifiers(type);
    if (type == nullptr) {
        return nullptr;
    }
    ...
    const SemanticType *pointee_type = strip_qualifiers(
...
bool is_addressable_expr(const Expr *expr,
                         const ConversionChecker &conversion_checker,
                         SemanticModel &semantic_model) {
    if (expr == nullptr) {
        return false;
    ...
    return expr_type != nullptr && strip_qualifiers(expr_type) != nullptr &&
           strip_qualifiers(expr_type)->get_kind() ==
               SemanticTypeKind::Function;
}

const SemanticType *
get_member_owner_type(const SemanticType *base_type,
    ...
    if (operator_text == "." && (conversion_checker.is_struct_type(base_type) ||
...
const SemanticType *
get_member_object_type(const SemanticType *base_type,
                       const std::string &operator_text,
                       const ConversionChecker &conversion_checker) {
    if (base_type == nullptr) {
        return nullptr;
    ...
    if (operator_text == "->") {
        if (conversion_checker.is_pointer_to_struct_type(base_type) ||
            conversion_checker.is_pointer_to_union_type(base_type)) {
            return static_cast<const PointerSemanticType *>(
                       unqualified_base_type)
                ->get_pointee_type();
        }
        if (unqualified_base_type != nullptr &&
            unqualified_base_type->get_kind() == SemanticTypeKind::Array) {
...
const SemanticType *
apply_member_object_qualifiers(const SemanticType *field_type,
                               const SemanticType *owner_object_type,
                               SemanticModel &semantic_model) {
    if (field_type == nullptr || owner_object_type == nullptr) {
        return field_type;
    }

    TopLevelQualifiers field_qualifiers =
        split_top_level_qualifiers(field_type);
...
resolve_completed_named_aggregate_type(const SemanticType *type,
    ...
    if (unqualified_type->get_kind() == SemanticTypeKind::Struct) {
        ...
        if (tag_symbol != nullptr &&
            ...
            const auto *completed_type =
                dynamic_cast<const StructSemanticType *>(
                    strip_qualifiers(tag_symbol->get_type()));
    ...
    if (unqualified_type->get_kind() == SemanticTypeKind::Union) {
        ...
        if (tag_symbol != nullptr &&
            ...
            tag_symbol->get_type() != nullptr) {
            const auto *completed_type =
                dynamic_cast<const UnionSemanticType *>(
                    strip_qualifiers(tag_symbol->get_type()));
            if (completed_type != nullptr &&
                !completed_type->get_fields().empty()) {
                return tag_symbol->get_type();
...
const SemanticType *
find_direct_union_field_type(const SemanticType *owner_type,
                             const std::string &field_name) {
    if (owner_type == nullptr ||
        owner_type->get_kind() != SemanticTypeKind::Union) {
        return nullptr;
    }
...
const SemanticFieldInfo *
find_direct_union_field_info(const SemanticType *owner_type,
                             const std::string &field_name) {
    if (owner_type == nullptr ||
        owner_type->get_kind() != SemanticTypeKind::Union) {
        return nullptr;
...
const SemanticType *
find_direct_struct_field_type(const SemanticType *owner_type,
                              const std::string &field_name) {
    if (owner_type == nullptr ||
...
const SemanticFieldInfo *
find_direct_struct_field_info(const SemanticType *owner_type,
                              const std::string &field_name) {
    if (owner_type == nullptr ||
        owner_type->get_kind() != SemanticTypeKind::Struct) {
        return nullptr;
...
std::optional<int>
get_bit_field_width_for_expr(const Expr *expr, SemanticModel &semantic_model,
                             const ConversionChecker &conversion_checker) {
    if (expr == nullptr) {
        return std::nullopt;
    }
    ...
    if (expr->get_kind() == AstKind::MemberExpr) {
        ...
        if (owner_type->get_kind() == SemanticTypeKind::Struct) {
            field_info = find_direct_struct_field_info(
                owner_type, member_expr->get_member_name());
        } else if (owner_type->get_kind() == SemanticTypeKind::Union) {
            field_info = find_direct_union_field_info(
                owner_type, member_expr->get_member_name());
        }
    ...
    if (expr->get_kind() == AstKind::BinaryExpr) {
        ...
        if (binary_expr->get_operator_text() == ",") {
            return get_bit_field_width_for_expr(
                binary_expr->get_rhs(), semantic_model, conversion_checker);
        }
        return std::nullopt;
    }
...
const SemanticType *
get_integer_promotion_type(const Expr *expr, SemanticModel &semantic_model,
                           const ConversionChecker &conversion_checker) {
    if (expr == nullptr) {
        return nullptr;
    }
...
const SemanticType *
get_common_integer_type(const Expr *lhs_expr, const SemanticType *lhs,
                        const Expr *rhs_expr, const SemanticType *rhs,
                        SemanticModel &semantic_model,
                        const ConversionChecker &conversion_checker) {
    detail::IntegerConversionService integer_conversion_service;
...
const SemanticType *
get_common_arithmetic_type(const Expr *lhs_expr, const SemanticType *lhs,
                           const Expr *rhs_expr, const SemanticType *rhs,
                           SemanticModel &semantic_model,
                           const ConversionChecker &conversion_checker) {
    detail::IntegerConversionService integer_conversion_service;
    ...
    return get_common_arithmetic_type(lhs, rhs, semantic_model,
                                      conversion_checker);
}
...
void analyze_statement_expr_stmt(const Stmt *stmt,
    ...
    case AstKind::DeclStmt: {
        ...
        for (const auto &decl : decl_stmt->get_declarations()) {
            decl_analyzer.analyze_decl(decl.get(), semantic_context,
                                       scope_stack);
        }
        return;
    }
    ...
    case AstKind::WhileStmt: {
        ...
        expr_analyzer.analyze_expr(while_stmt->get_condition(),
                                   semantic_context, scope_stack);
...
void ExprAnalyzer::analyze_expr(const Expr *expr,
    ...
    case AstKind::IdentifierExpr: {
        ...
        const SemanticSymbol *symbol =
            scope_stack.lookup(identifier_expr->get_name());
        if (symbol == nullptr) {
    ...
    case AstKind::IntegerLiteralExpr:
        semantic_model.bind_node_type(
            expr,
            get_integer_literal_semantic_type(
                static_cast<const IntegerLiteralExpr *>(expr), semantic_model));
        if (const auto parsed_value = parse_integer_literal(
                static_cast<const IntegerLiteralExpr *>(expr)
                    ->get_value_text());
            parsed_value.has_value()) {
            constant_evaluator_.bind_integer_constant_value(expr, *parsed_value,
                                                            semantic_context);
        }
        return;
    case AstKind::FloatLiteralExpr:
        semantic_model.bind_node_type(
            expr,
            get_float_literal_semantic_type(
                static_cast<const FloatLiteralExpr *>(expr), semantic_model));
        return;
    ...
    case AstKind::StringLiteralExpr: {
        const auto *string_literal =
            static_cast<const StringLiteralExpr *>(expr);
        const auto *char_type = semantic_model.own_type(
            std::make_unique<BuiltinSemanticType>("char"));
    ...
    case AstKind::SizeofTypeExpr: {
        const auto *sizeof_type_expr =
            static_cast<const SizeofTypeExpr *>(expr);
        ...
        const SemanticType *target_type =
            type_resolver_.resolve_type(sizeof_type_expr->get_target_type(),
                                        semantic_context, &scope_stack);
        ...
        constant_evaluator_.bind_integer_constant_value(
            sizeof_type_expr, static_cast<long long>(*type_size),
            semantic_context);
        return;
    }
    ...
    case AstKind::UnaryExpr: {
        ...
        if (unary_expr->get_operator_text() == "&&") {
            ...
            semantic_context.record_goto_reference(
                label_expr->get_name(), unary_expr->get_source_span());
            const auto *void_type = semantic_model.own_type(
                std::make_unique<BuiltinSemanticType>("void"));
            semantic_model.bind_node_type(
                unary_expr,
        ...
        if (unary_expr->get_operator_text() == "sizeof") {
            semantic_model.bind_node_type(
                unary_expr, get_size_t_semantic_type(semantic_model));
            const SemanticType *layout_operand_type =
                resolve_completed_named_aggregate_type(operand_type,
                                                       scope_stack);
            const auto operand_size =
                get_semantic_type_size(layout_operand_type);
        ...
        if (unary_expr->get_operator_text() == "&") {
            if (!is_addressable_expr(unary_expr->get_operand(),
                                     conversion_checker_, semantic_model)) {
                add_error(semantic_context,
            ...
            semantic_model.bind_node_type(
                unary_expr,
                semantic_model.own_type(
                    std::make_unique<PointerSemanticType>(operand_type)));
            return;
        }
        if (unary_expr->get_operator_text() == "*") {
            ...
            semantic_model.bind_node_type(
                unary_expr, resolve_completed_named_aggregate_type(
                                static_cast<const PointerSemanticType *>(
                                    unqualified_operand_type)
                                    ->get_pointee_type(),
                                scope_stack));
            return;
        }
        if (unary_expr->get_operator_text() == "!") {
            const SemanticType *logical_operand_type =
                conversion_checker_.get_decayed_type(operand_type,
                                                     semantic_model);
            if (!conversion_checker_.is_scalar_type(logical_operand_type)) {
                add_error(semantic_context,
            ...
            semantic_model.bind_node_type(
                unary_expr, get_int_semantic_type(semantic_model));
        ...
        if (unary_expr->get_operator_text() == "~") {
            ...
            const SemanticType *result_type = get_integer_promotion_type(
                unary_expr->get_operand(), semantic_model, conversion_checker_);
        ...
        if (unary_expr->get_operator_text() == "+" ||
            ...
            semantic_model.bind_node_type(
                unary_expr, get_common_arithmetic_type(
    ...
    case AstKind::PrefixExpr: {
        ...
        if (!conversion_checker_.is_assignable_expr(
    ...
    case AstKind::PostfixExpr: {
        const auto *postfix_expr = static_cast<const PostfixExpr *>(expr);
        analyze_expr(postfix_expr->get_operand(), semantic_context,
                     scope_stack);
        if (postfix_expr->get_operand() != nullptr &&
            postfix_expr->get_operand()->get_kind() ==
    ...
    case AstKind::BinaryExpr: {
        ...
        if (operator_text == "%") {
            ...
            if (lhs_constant.has_value() && rhs_constant.has_value() &&
                *rhs_constant != 0) {
                constant_evaluator_.bind_integer_constant_value(
                    binary_expr, *lhs_constant % *rhs_constant,
                    semantic_context);
            }
            return;
        }
        ...
        if (operator_text == "<<" || operator_text == ">>" ||
            ...
            if (operator_text == "<<" || operator_text == ">>") {
                result_type = get_integer_promotion_type(binary_expr->get_lhs(),
                                                         semantic_model,
                                                         conversion_checker_);
            } else {
        ...
        if (operator_text == "&&" || operator_text == "||") {
            ...
            semantic_model.bind_node_type(
                binary_expr, get_int_semantic_type(semantic_model));
            if (lhs_constant.has_value() && rhs_constant.has_value()) {
        ...
        if (operator_text == "<" || operator_text == "<=" ||
            ...
            if (conversion_checker_.is_integer_like_type(lhs_type) &&
                conversion_checker_.is_integer_like_type(rhs_type) &&
                conversion_checker_.should_warn_sign_compare(lhs_type, rhs_type,
                                                             semantic_model)) {
        ...
        if (operator_text == "==" || operator_text == "!=") {
            ...
            semantic_model.bind_node_type(
                binary_expr, get_int_semantic_type(semantic_model));
            if (lhs_constant.has_value() && rhs_constant.has_value()) {
                const long long result = operator_text == "=="
    ...
    case AstKind::CastExpr: {
        ...
        const SemanticType *target_type = type_resolver_.resolve_type(
            cast_expr->get_target_type(), semantic_context, &scope_stack);
        ...
        if (!conversion_checker_.is_castable_type(target_type,
                                                  cast_operand_type)) {
            add_error(semantic_context,
                      "unsupported cast between operand types",
    ...
    case AstKind::ConditionalExpr: {
        ...
        const SemanticType *condition_value_type =
            conversion_checker_.get_decayed_type(condition_type,
                                                 semantic_model);
        if (!conversion_checker_.is_scalar_type(condition_value_type)) {
            add_error(semantic_context,
        ...
        if (conversion_checker_.is_arithmetic_type(true_type) &&
            ...
            if (conversion_checker_.is_assignable_type(true_value_type,
                                                       false_value_type)) {
                result_type = true_value_type;
            } else if (conversion_checker_.is_assignable_type(
                           false_value_type, true_value_type)) {
                result_type = false_value_type;
            }
        }
    ...
    case AstKind::AssignExpr: {
        ...
        if (target_type != nullptr && value_type != nullptr) {
            if (assign_expr->get_operator_text() == "=") {
                ...
                } else if (assign_expr->get_value()->get_kind() !=
                               AstKind::CastExpr &&
                           conversion_checker_
                ...
                if (binary_operator == "+" || binary_operator == "-" ||
                    ...
                    if (!conversion_checker_.is_arithmetic_type(target_type) ||
                        ...
                        add_error(semantic_context,
                           ...
                           binary_operator == ">>" || binary_operator == "&" ||
                           binary_operator == "|" || binary_operator == "^") {
                    if (!conversion_checker_.is_integer_like_type(
                            target_type) ||
                        !conversion_checker_.is_integer_like_type(value_type)) {
                        add_error(semantic_context,
                                  "compound assignment '" +
    ...
    case AstKind::CallExpr: {
        ...
        if (const IdentifierExpr *callee = get_callee_identifier(call_expr);
            ...
            semantic_model.bind_node_type(
                call_expr, semantic_model.get_node_type(argument));
            return;
        }
        if (const IdentifierExpr *callee = get_callee_identifier(call_expr);
            callee != nullptr &&
            ...
            const long long is_compatible =
                lhs_type != nullptr && rhs_type != nullptr &&
                    ...
                    : 0LL;
            semantic_model.bind_node_type(
                call_expr, get_int_semantic_type(semantic_model));
            constant_evaluator_.bind_integer_constant_value(
        ...
        if ((variadic && argument_count < fixed_parameter_count) ||
            ...
            for (std::size_t index = 0; index < fixed_parameter_count;
                 ++index) {
                const SemanticType *argument_type =
                    semantic_model.get_node_type(
                ...
                if (argument_type != nullptr && parameter_type != nullptr &&
                    ...
                    if (conversion_checker_.is_incompatible_pointer_assignment(
                        ...
                        add_warning(semantic_context,
                                    "function call argument uses incompatible "
                                    "pointer type",
                                    call_expr->get_arguments()[index]
                                        ->get_source_span(),
                                    warning_options::kIncompatiblePointerTypes);
                    } else {
                        add_error(semantic_context,
                                  "function call argument type does not match "
                                  "declaration",
                                  call_expr->get_arguments()[index]
                                      ->get_source_span());
                        break;
    ...
    case AstKind::MemberExpr: {
        ...
        if (const SemanticType *field_type = find_direct_union_field_type(
                owner_type, member_expr->get_member_name());
            field_type != nullptr) {
            semantic_model.bind_node_type(
        ...
        if (const SemanticType *field_type = find_direct_struct_field_type(
                owner_type, member_expr->get_member_name());
            field_type != nullptr) {
            semantic_model.bind_node_type(
                expr, apply_member_object_qualifiers(
            ...
            if (struct_symbol != nullptr &&
                ...
                for (const auto &field : struct_decl->get_fields()) {
                    ...
                    if (field_decl->get_name() ==
                        ...
                        semantic_model.bind_node_type(
                            expr, apply_member_object_qualifiers(
                                      resolved_field_type, owner_object_type,
                                      semantic_model));
                        return;
                    }
                }
        ...
        if (owner_type->get_kind() == SemanticTypeKind::Union) {
            ...
            if (union_symbol != nullptr &&
                ...
                const auto *union_decl = static_cast<const UnionDecl *>(
                    union_symbol->get_decl_node());
    ...
    case AstKind::StatementExpr: {
        ...
        semantic_model.bind_node_type(
            statement_expr, result_type != nullptr
                                ? result_type
