...
class AstBuilder {
  public:
    ...
    build(const AstBuilderContext &context) const;

  private:
    void
    collect_top_level_items(const ParseTreeNode *node,
    ...
    build_function_decl(const ParseTreeNode *node) const;
    ParsedAttributeList build_decl_attributes(const ParseTreeNode *node) const;
    std::vector<std::unique_ptr<Decl>>
    build_parameters(const ParseTreeNode *node) const;
    std::vector<std::unique_ptr<Decl>>
    build_decl_group(const ParseTreeNode *node) const;
    std::vector<std::unique_ptr<Decl>>
    build_const_decls(const ParseTreeNode *node) const;
    std::vector<std::unique_ptr<Decl>>
    build_var_decls(const ParseTreeNode *node) const;
    std::vector<std::unique_ptr<Decl>>
    build_typedef_decls(const ParseTreeNode *node) const;
    std::unique_ptr<StructDecl>
    build_struct_decl(const ParseTreeNode *node) const;
    std::unique_ptr<UnionDecl>
    build_union_decl(const ParseTreeNode *node) const;
    std::unique_ptr<EnumDecl> build_enum_decl(const ParseTreeNode *node) const;
    std::unique_ptr<TypeNode>
    build_return_type(const ParseTreeNode *node,
                      bool include_inline_enum_enumerators = false) const;
    // NOLINTNEXTLINE(bugprone-easily-swappable-parameters)
    std::unique_ptr<TypeNode> build_declared_type(
        const ParseTreeNode *type_specifier, const ParseTreeNode *declarator,
        bool pointee_is_const = false, bool pointee_is_volatile = false,
        bool include_inline_enum_enumerators = false) const;
    std::unique_ptr<TypeNode>
    build_declarator_type(std::unique_ptr<TypeNode> base_type,
                          const ParseTreeNode *declarator) const;
    std::unique_ptr<TypeNode>
    build_pointer_type(std::unique_ptr<TypeNode> base_type,
                       const ParseTreeNode *pointer) const;
    std::unique_ptr<TypeNode>
    build_direct_declarator_type(std::unique_ptr<TypeNode> base_type,
    ...
    std::string
    build_string_literal_sequence_text(const ParseTreeNode *node) const;
    std::vector<std::unique_ptr<Decl>>
    build_struct_fields(const ParseTreeNode *node) const;
    std::vector<std::unique_ptr<Decl>>
    build_union_fields(const ParseTreeNode *node) const;
    std::vector<std::unique_ptr<Decl>>
    build_enumerators(const ParseTreeNode *node) const;
    std::vector<std::unique_ptr<Expr>>
    collect_declarator_dimensions(const ParseTreeNode *node) const;
    std::string extract_declarator_name(const ParseTreeNode *node) const;
    std::unique_ptr<BlockStmt>
    build_block_stmt(const ParseTreeNode *node) const;
    ...
    std::vector<std::unique_ptr<Expr>>
    build_argument_exprs(const ParseTreeNode *node) const;
    ...
    build_init_list_expr(const ParseTreeNode *node) const;
    InitListExpr::DesignatorPath
    build_initializer_designator_path(const ParseTreeNode *node) const;
    std::unique_ptr<TypeNode>
    build_cast_target_type(const ParseTreeNode *node) const;
