...
void print_usage(const char *program_name) {
    std::cout
        ...
        << "  -o <file>          Output path\n"
        << "  --target <triple>  Target triple (default: aarch64-unknown-linux-gnu)\n"
        << "  -fPIC              Enable position-independent code generation\n"
        << "  -g                 Enable basic debug info generation\n"
        << "  --help             Show this help message\n"
        << "  --version          Show version information\n";
}

void print_version() { std::cout << "sysycc-aarch64c version 0.1.0\n"; }
...
void print_diagnostics(
    ...
    for (const sysycc::AArch64CodegenDiagnostic &diagnostic : diagnostics) {
...
int main(int argc, char **argv) {
    ...
    if (emit_asm) {
        const sysycc::AArch64AsmCompileResult result =
            is_bc ? sysycc::compile_bc_file_to_asm(request)
                  : sysycc::compile_ll_file_to_asm(request);
        print_diagnostics(result.diagnostics);
        if (result.status != sysycc::AArch64CodegenStatus::Success) {
            return 1;
        }
    ...
    const sysycc::AArch64ObjectCompileResult result =
