...
bool simplify_conditional_branch_condition(CoreIrContext &context,
    ...
    if (std::optional<BooleanCompareMatch> match =
        ...
        if (!(compare != nullptr && compare_type != nullptr &&
              ...
              compare == cond_jump->get_condition())) {
            changed = true;
...
bool run_instcombine_on_function(CoreIrContext &context, CoreIrFunction &function) {
    bool changed = false;
    std::vector<CoreIrInstruction *> worklist =
        collect_function_instructions(function);
    while (!worklist.empty()) {
        CoreIrInstruction *instruction = worklist.back();
        worklist.pop_back();
        if (instruction == nullptr ||
        ...
        if (simplify_instruction(context, *block, *instruction)) {
            changed = true;
            std::vector<CoreIrInstruction *> refreshed =
                collect_function_instructions(function);
            worklist.insert(worklist.end(), refreshed.begin(), refreshed.end());
        }
    }
    return changed;
}
...
PassResult CoreIrInstCombinePass::Run(CompilerContext &context) {
    ...
    for (const auto &function : module->get_functions()) {
        if (function != nullptr &&
            run_instcombine_on_function(*core_ir_context, *function)) {
