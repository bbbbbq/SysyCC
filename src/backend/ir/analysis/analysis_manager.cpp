...
template <typename CacheEntryT>
void clear_escape_dependent_function_analyses(CacheEntryT &cache_entry) {
    cache_entry.escape_analysis.reset();
    cache_entry.alias_analysis.reset();
    cache_entry.memory_ssa_analysis.reset();
}

template <typename CacheEntryT>
bool cached_function_analyses_empty(const CacheEntryT &cache_entry) {
    return cache_entry.cfg_analysis == nullptr &&
           cache_entry.dominator_tree_analysis == nullptr &&
           cache_entry.dominance_frontier_analysis == nullptr &&
           cache_entry.promotable_stack_slot_analysis == nullptr &&
           cache_entry.loop_info_analysis == nullptr &&
           cache_entry.induction_var_analysis == nullptr &&
           cache_entry.scalar_evolution_lite_analysis == nullptr &&
           cache_entry.escape_analysis == nullptr &&
           cache_entry.alias_analysis == nullptr &&
           cache_entry.memory_ssa_analysis == nullptr &&
           cache_entry.function_effect_summary == nullptr;
}
...
CoreIrAnalysisManager::get_or_compute_call_graph(CoreIrModule &module) {
    CachedModuleAnalyses &cache_entry =
        get_or_create_module_cache_entry(module);
    if (cache_entry.call_graph_analysis == nullptr) {
        CoreIrCallGraphAnalysis analysis;
        cache_entry.call_graph_analysis =
            std::make_unique<CoreIrCallGraphAnalysisResult>(
                analysis.Run(module));
...
CoreIrAnalysisManager::get_or_compute_induction_vars(CoreIrFunction &function) {
    ...
    if (cache_entry.induction_var_analysis == nullptr) {
        ...
        cache_entry.induction_var_analysis =
            std::make_unique<CoreIrInductionVarAnalysisResult>(analysis.Run(
                function, cfg_analysis, dominator_tree, loop_info));
        cache_entry.scalar_evolution_lite_analysis.reset();
        bump_compute_count(compute_counts_, function,
    ...
    if (cache_entry.scalar_evolution_lite_analysis == nullptr) {
        ...
        cache_entry.scalar_evolution_lite_analysis =
            std::make_unique<CoreIrScalarEvolutionLiteAnalysisResult>(
                analysis.Run(function, cfg_analysis, loop_info,
                             induction_vars));
        bump_compute_count(compute_counts_, function,
...
CoreIrAnalysisManager::get_or_compute_escape_analysis(
    CoreIrFunction &function) {
    CachedFunctionAnalyses &cache_entry = get_or_create_cache_entry(function);
    if (cache_entry.escape_analysis == nullptr) {
        const CoreIrFunctionAttrsAnalysisResult *function_attrs = nullptr;
        if (CoreIrModule *module = function.get_parent(); module != nullptr) {
            function_attrs = &get_or_compute_function_attrs(*module);
        }

        CoreIrEscapeAnalysis analysis;
        cache_entry.escape_analysis =
            std::make_unique<CoreIrEscapeAnalysisResult>(
                analysis.Run(function, function_attrs));
        cache_entry.alias_analysis.reset();
        cache_entry.memory_ssa_analysis.reset();
        bump_compute_count(compute_counts_, function,
                           CoreIrAnalysisKind::EscapeAnalysis);
    }
    return *cache_entry.escape_analysis;
}
...
CoreIrAnalysisManager::get_or_compute_alias_analysis(CoreIrFunction &function) {
    ...
    if (cache_entry.alias_analysis == nullptr) {
        (void)get_or_compute_escape_analysis(function);
...
CoreIrAnalysisManager::get_or_compute_memory_ssa(CoreIrFunction &function) {
    ...
    if (cache_entry.memory_ssa_analysis == nullptr) {
        ...
        cache_entry.memory_ssa_analysis =
            std::make_unique<CoreIrMemorySSAAnalysisResult>(analysis.Run(
                function, cfg_analysis, dominator_tree, dominance_frontier,
                effect_summary, alias_analysis));
        bump_compute_count(compute_counts_, function,
...
void CoreIrAnalysisManager::invalidate(CoreIrAnalysisKind kind) noexcept {
    ...
    case CoreIrAnalysisKind::CallGraph:
        ...
        for (auto it = function_cache_.begin(); it != function_cache_.end();) {
            clear_escape_dependent_function_analyses(it->second);
            if (cached_function_analyses_empty(it->second)) {
    ...
    for (auto it = function_cache_.begin(); it != function_cache_.end();) {
        ...
        case CoreIrAnalysisKind::ScalarEvolutionLite:
            ...
            break;
        case CoreIrAnalysisKind::EscapeAnalysis:
            it->second.escape_analysis.reset();
...
void CoreIrAnalysisManager::invalidate(CoreIrModule &module,
    ...
    case CoreIrAnalysisKind::CallGraph:
        ...
        for (const auto &function_ptr : module.get_functions()) {
            ...
            auto function_it = function_cache_.find(function);
            if (function_it == function_cache_.end()) {
                continue;
            }
            clear_escape_dependent_function_analyses(function_it->second);
            if (cached_function_analyses_empty(function_it->second)) {
                function_cache_.erase(function_it);
            }
        }
        break;
    case CoreIrAnalysisKind::FunctionAttrs:
