...
PassResult fail_missing_llvm_ir_artifacts(CompilerContext &context,
    ...
    const std::string message =
        ...
        " requires lowered LLVM IR artifacts before native RISC-V64 codegen";
    context.get_diagnostic_engine().add_error(DiagnosticStage::Compiler,
                                              message);
    return PassResult::Failure(message);
}

DiagnosticStage map_api_diagnostic_stage(
    const Riscv64CodegenDiagnostic &diagnostic) {
    (void)diagnostic;
...
void append_api_diagnostics(
    ...
    const std::vector<Riscv64CodegenDiagnostic> &diagnostics) {
    DiagnosticEngine &diagnostic_engine = context.get_diagnostic_engine();
    for (const Riscv64CodegenDiagnostic &diagnostic : diagnostics) {
        switch (diagnostic.severity) {
        case Riscv64CodegenDiagnosticSeverity::Warning:
            diagnostic_engine.add_warning(map_api_diagnostic_stage(diagnostic),
                                          diagnostic.message);
            break;
        case Riscv64CodegenDiagnosticSeverity::Note:
            diagnostic_engine.add_note(map_api_diagnostic_stage(diagnostic),
                                       diagnostic.message);
            break;
        case Riscv64CodegenDiagnosticSeverity::Error:
        default:
...
std::filesystem::path get_object_output_file_path(
...
Riscv64CodegenFileRequest build_codegen_request(
    const CompilerContext &context, const std::string &input_file_path) {
    Riscv64CodegenFileRequest request;
...
PassKind Riscv64AsmGenPass::Kind() const { return PassKind::CodeGen; }

const char *Riscv64AsmGenPass::Name() const { return "Riscv64AsmGenPass"; }

PassResult Riscv64AsmGenPass::Run(CompilerContext &context) {
    ...
    if (context.get_emit_asm()) {
        Riscv64AsmCompileResult asm_result =
        ...
        if (asm_result.status != Riscv64CodegenStatus::Success) {
            return PassResult::Failure(
                "failed to generate native RISC-V64 assembly from LLVM IR artifacts");
        }

        context.set_asm_result(std::make_unique<AsmResult>(
            AsmTargetKind::Riscv64, asm_result.asm_text));
    ...
    Riscv64ObjectCompileResult object_result =
    ...
    if (object_result.status != Riscv64CodegenStatus::Success) {
        return PassResult::Failure(
            "failed to generate native RISC-V64 object output from LLVM IR artifacts");
    }

    context.set_object_result(std::make_unique<ObjectResult>(
        ObjectTargetKind::ElfRiscv64, object_result.object_bytes));
...
extern "C" sysycc::Pass *sysycc_create_riscv64_asm_gen_pass() {
    return new sysycc::Riscv64AsmGenPass();
}
